...
const RESOLUTION_MAP: Record<RecordingResolution, { width: number; height: number } | undefined> = {
...
export function useScreenCapture() {
  ...
  const startCapture = useCallback(
    async (resolution: RecordingResolution = "1080p"): Promise<MediaStream | null> => {
      setError(null);
      const size = RESOLUTION_MAP[resolution];
      try {
        const stream = await navigator.mediaDevices.getDisplayMedia({
          video: size
            ? { displaySurface: "monitor", width: size.width, height: size.height }
